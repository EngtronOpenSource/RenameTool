...
void FRenameToolHelper::SetAssetNewName(UObject* Asset, int Index)
{
	...
	switch (RenameInformation.GetRenameAction())
	{
		...
		case ERenameAction::ToLowerCase:
		{
			...
			{
				char UpperChar = 'A' - 'a' + Lowerchar;
				FString NewString = FString("_").AppendChar(Lowerchar);
				FString OldString = FString("_").AppendChar(UpperChar);
		...
		case ERenameAction::ToUpperCase:
		{
			...
			{
				char UpperChar = 'A' - 'a' + Lowerchar;
				FString OldString = FString("_").AppendChar(Lowerchar);
				FString NewString = FString("_").AppendChar(UpperChar);
				NewName = NewName.Replace(*OldString, *NewString);
			}
		}break;
	}
